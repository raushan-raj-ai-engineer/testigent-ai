...
export class AiGateway {
  ...
  async proposeLocator(request: AiHealingRequest): Promise<AiHealingResponse | undefined> {
    ...
    if (this.healingCalls >= this.healingLimit) {
      this.audit.record({ purpose: 'healing', status: 'budget-blocked', message: `AI healing call limit ${this.healingLimit} reached.` });
      return undefined;
    }
    this.healingCalls += 1;
    ...
    try {
      ...
      this.audit.record({ purpose: 'healing', status: 'error', latencyMs: Date.now() - started, message: error instanceof Error ? error.name : 'ProviderError' });
      throw error;
    }
  }

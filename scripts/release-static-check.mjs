...
try {
  ...
  const proposalReviewV192 = fs.readFileSync(path.join(root, 'src/framework/intelligence/review/proposal.review.ts'), 'utf8');
  ...
  const postCertClosureV192 = fs.readFileSync(path.join(root, 'docs/69-v1.9.2-POST-CERT-CORRECTIVE-CLOSURE.md'), 'utf8');
  ...
  if (!pathPolicyV192.includes('resolveProjectScopedPath') || !pathPolicyV192.includes('resolveProjectMutationPath') || !pathPolicyV192.includes('fs.lstatSync(cursor)') || !mcpSecurityV192.includes('resolveProjectScopedPath')) issues.push('v1.9.2 path security must anchor reads/mutations to selected-project boundaries and inspect dangling directory entries with lstat');
  if (!proposalReviewV192.includes('writeProjectFileNoFollow') || !proposalReviewV192.includes('fsConstants.O_EXCL') || !proposalReviewV192.includes('fsConstants.O_NOFOLLOW')) issues.push('v1.9.2 promotion mutation boundary must use exclusive/no-follow file creation semantics');
  if (!schemaValidatorV192.includes("typeof schema === 'boolean'") || !schemaValidatorV192.includes('if (isObject(value))') || !schemaValidatorV192.includes('if (Array.isArray(value))') || !schemaValidatorV192.includes('if (is31)') || !openApiLoaderV192.includes("allOf: [resolved, siblings]") || !responseContract.includes('schema === undefined')) issues.push('v1.9.2 OpenAPI validation must retain boolean/$ref semantics plus omitted-type runtime constraints and OpenAPI 3.1 null behavior');
  if (!breakingDetector.includes("item.in === 'header'") || !breakingDetector.includes('toLowerCase()') || !breakingDetector.includes('INCOMPLETE_COMPARISON') || !breakingDetector.includes('REQUEST_CONSTRAINT_TIGHTENED') || !breakingDetector.includes('effectivePropertySchema') || !breakingDetector.includes('propertyKeys')) issues.push('v1.9.2 compatibility detection must retain directional verdicts and effective-property comparison when declarations disappear');
  ...
  for (const marker of ['R01 public response contract', 'Post-certification reviewer counterexamples', 'R02 query names remain case-sensitive', 'R02 property-removal compatibility', 'R04 selected-project boundary', 'R04 promotion workflow refuses', 'R09 complete dashboard regeneration', 'R07 legacy classifier hints', 'R10 SQL lexical handling', 'R03 strict TLS', 'R12 real child-process stdio', 'R11 CSV output']) if (!rereviewClosureV192.includes(marker)) issues.push(`v1.9.2 expanded re-review closure missing production-boundary contract: ${marker}`);
  if (!postCertClosureV192.includes('ba711591a5ca4ab47bb380ad192bd6e1af75368a4887ae253f34a70d4f320992') || !postCertClosureV192.includes('post-certification corrective candidate') || !postCertClosureV192.includes('R01') || !postCertClosureV192.includes('R02') || !postCertClosureV192.includes('R04')) issues.push('v1.9.2 post-certification corrective handoff must preserve review provenance and bounded release status');

...
Core shard business bundles, technical blobs, AI business/audit bundles and the final report artifact include this identity in their artifact names. A normal first attempt resolves entirely from the current attempt. For GitHub `rerun --failed`, the current attempt may not expose all evidence through the default artifact backend view. The merge therefore uses authenticated `actions/download-artifact` workflow-run lookup (`github-token`, `repository`, `run-id`) to acquire the **same workflow-run family** (`github.run_id`), then runs `npm run ci:report:rerun:resolve` before validation. The resolver chooses the newest valid attempt independently per core shard and, when the AI job succeeds, requires the AI source to be from the current attempt. It produces `_rerun-resolution.json` as an auditable provenance decision and carries the matching technical blobs from the same source attempts.
...
Intermediate report artifacts are retained whenever a required core or AI lane fails. Cleanup is allowed only after required execution lanes, merge, final bundle validation and final report upload all succeed. This prevents a failed first attempt from deleting the evidence a later partial rerun must verify.
